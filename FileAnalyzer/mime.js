...
for (signature in signatures) {
    ...
    dotnetType = dotnetType + `\t\t\t\t".${signature}", // ${signatures[signature].mime}\n`
    dotnetType = dotnetType + "\t\t\t\tnew List<FileSignature>() {\n"

    for (var n = 0; n < signatures[signature].signs.length; n++) {
        var sign = signatures[signature].signs[n].split(',')

        var duos = []
        var joinedBytes = sign[1]

        for (var i = 0; i < joinedBytes.length; i += 2) {
            duos.push(`${joinedBytes[i]}${joinedBytes[i + 1]}`)
        }

        dotnetType = dotnetType + `\t\t\t\t\tnew FileSignature() { Offset = ${sign[0]}, Bytes = new byte[] { `

        for (var i = 0; i < duos.length; i++) {
            if (i != duos.length - 1) {
                // add comma when it is not last element
                dotnetType = dotnetType + `0x${duos[i]}, ` // add all the 0x...
            } else {
                dotnetType = dotnetType + `0x${duos[i]} ` // add all the 0x...
            }
        }

        if (n != signatures[signature].signs.length - 1) {
            // add comma when it is not last element
            dotnetType = dotnetType + "} },\n"
        } else {
            dotnetType = dotnetType + "} }\n"
...
console.log(dotnetType)
// after the mime types have been converted into a C# dictionary, the string can be copied from the console